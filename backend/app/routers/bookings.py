...
from app.services.booking_service import check_counselor_availability, expire_booking_if_unpaid
...
async def create_booking(
    ...
    is_available = await check_counselor_availability(
        db, booking_data.counselor_id, booking_data.scheduled_start, booking_data.scheduled_end
    )
    if not is_available:
        raise HTTPException(status_code=400, detail="The counselor is not available during this time slot.")
...
async def get_my_bookings(
    ...
    """
    result = await db.execute(select(Booking).where(Booking.client_id == current_user.id))
