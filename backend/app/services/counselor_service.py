...
async def get_active_counselors(db: AsyncSession):
    ...
    stmt = select(CounselorProfile).where(
...
async def get_counselor_by_user_id(db: AsyncSession, user_id: str):
    ...
    stmt = select(CounselorProfile).where(
        CounselorProfile.user_id == user_id
    ).options(selectinload(CounselorProfile.user))
    
    result = await db.execute(stmt)
    return result.scalar_one_or_none()

async def create_counselor_profile(db: AsyncSession, user_id: str, profile_data: CounselorProfileCreate):
    ...
    db_profile = CounselorProfile(
        ...
        credentials_url=str(profile_data.credentials_url) if profile_data.credentials_url else None,
    ...
    stmt = select(CounselorProfile).where(CounselorProfile.id == db_profile.id).options(selectinload(CounselorProfile.user))
    result = await db.execute(stmt)
    return result.scalar_one()

async def update_counselor_profile(db: AsyncSession, profile: CounselorProfile, update_data: CounselorProfileUpdate):
